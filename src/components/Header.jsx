...
export default function Header() {
  ...
  let letterStyles =
    "text-white text-center hover:text-teal-400 transition duration-300 ease lg:text-slate-400 lg:hover:text-black";

  let mobileDropdownContainerStyles =
    "w-11/12 overflow-hidden mx-auto left-0 right-0 gap-y-4 bg-violet-950 rounded-md absolute grid transition-all duration-300 ease" +
    (toggleDropdown ? " h-72 p-8" : " h-0 ");

  let desktopDropdownContainerStyles = "w-full flex justify-between";
  function toggle() {
    setToggleDropdown(!toggleDropdown);
  }

  return (
    <header className="w-full lg:flex lg:px-12 lg:py-8 lg:gap-x-8 xl:px-24">
      <div className="w-full px-4 py-8 flex items-center justify-between text-gray-500 relative md:px-12 lg:w-auto lg:h-auto lg:p-0">
        <a href="/">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-32 h-8 fill-slate-700 hover:fill-black transition duration-300 ease"
          >
        ...
        <button className="cursor-pointer lg:hidden" onClick={toggle}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-8 w-8 hover:stroke-black active:stroke-slate-300 
      ...
      <nav
        className={
          isDesktop
            ? desktopDropdownContainerStyles
            : mobileDropdownContainerStyles
        }
      >
        <ul className="flex flex-col items-center gap-y-4 lg:flex-row lg:gap-x-6">
        ...
        <hr className="w-full h-0.5 border-none bg-slate-300 lg:hidden" />

        <div className="flex flex-col gap-y-4 items-center lg:flex-row lg:gap-x-6">
          <button className={`${letterStyles} w-full lg:w-auto`}>Login</button>
          <button
            className="w-full px-6 py-2 text-white bg-teal-400 rounded-2xl
           hover:bg-teal-300 active:bg-teal-400 lg:w-auto"
          >
            Sign Up
          </button>
        </div>
